...
export default async function Home() {
  ...
  return (
    <Typography component="div">
      ...
      <Typography className='effect-transform-hover' component="div" sx={{ backgroundColor: '#3cc4ff', borderRadius: '7px', overflow: 'hidden', boxShadow: 9 }}>
        <Typography component="div" sx={{ background: `url(${banner.src}) top no-repeat `, backgroundSize: 'contain' }}>
          <Grid container className='justify-between' sx={{ paddingTop: { xs: '80px', sm: '100px', md: '200px' }, paddingLeft: { xs: '2px', md: '25px' }, paddingRight: { xs: '2px', md: '25px' } }}>
            {
              data.map((item: any) =>
                <Grid xs={12} sm={6} md={4} className='px-3' key={item.id}>
